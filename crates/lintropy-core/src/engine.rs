...
impl<'a> RulesByLanguage<'a> {
    fn new(config: &'a Config) -> Result<Self> {
        ...
        for rule in &config.rules {
            let Some(language) = rule.language.as_deref() else {
                continue;
            };
            ...
            if language == lintropy_langs::Language::Rust.name() {
                rust.push(scoped);
            }
        }
        Ok(Self { rust })
...
fn run_file(path: &Path, rules_by_language: &RulesByLanguage<'_>) -> Result<Vec<Diagnostic>> {
    ...
    for scoped_rule in scoped_rules {
        ...
        for query_match in cursor.matches(&query_rule.query, root, src.as_slice()) {
            ...
            if !pattern_predicates
                .iter()
                .all(|predicate| predicate.apply(&query_match, &query_rule.query, &root, &src))
            {
                continue;
            }
...
fn build_diagnostic(
    ...
    let captures = capture_map(query_match.captures, &query_rule.query, src)?;
    let span_node = query_rule
        .query
        .capture_index_for_name("match")
        .and_then(|match_capture| query_match.nodes_for_capture_index(match_capture).next())

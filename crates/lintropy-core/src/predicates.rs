...
impl CustomPredicate {
    pub fn apply(&self, query_match: &QueryMatch<'_, '_>, _query: &Query, _root: &Node<'_>, src: &[u8]) -> bool {
        match self {
            ...
            CustomPredicate::HasPrecedingComment { capture, pattern } => {
                capture_nodes(query_match, *capture).any(|node| has_preceding_comment(node, pattern, src))
            }
            CustomPredicate::NotHasPrecedingComment { capture, pattern } => {
                capture_nodes(query_match, *capture).all(|node| !has_preceding_comment(node, pattern, src))
            }
        }
    }
}
...
pub fn parse_general_predicates(query: &Query) -> Result<Vec<CustomPredicate>> {
    Ok(parse_general_predicates_by_pattern(query)?.into_iter().flatten().collect())
}

pub(crate) fn parse_general_predicates_by_pattern(query: &Query) -> Result<Vec<Vec<CustomPredicate>>> {
...
fn capture_nodes<'a>(query_match: &'a QueryMatch<'a, 'a>, capture: u32) -> impl Iterator<Item = Node<'a>> + 'a {
    query_match
        .captures
        .iter()
...
mod tests {
    ...
    fn applies_ancestor_predicate() {
        ...
        let m = cursor
            ...
            .next()
            .unwrap();
        let recv = m.nodes_for_capture_index(query.capture_index_for_name("recv").unwrap()).next().unwrap();
